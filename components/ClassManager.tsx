
import React, { useState, useMemo, useEffect } from 'react';
import { useClassManager } from '../presentation/hooks/useClassManager';
import { useSettingsManager } from '../presentation/hooks/useSettingsManager';
import { Users, Plus, Trash2, Loader2, BookOpen, AlertTriangle, RotateCcw, Building2, Eye, Edit2, CheckCircle, Lock, ChevronRight, ChevronLeft, Home, X } from 'lucide-react';
import { SchoolClass } from '../types';
import ClassDetails from './ClassDetails';
import StudentManager from './StudentManager';
import ProfessorManager from './ProfessorManager';
import GradeManager from './GradeManager';
import TestManager from './TestManager';
import ConfirmationModal from './ConfirmationModal';
import { getFriendlyErrorMessage } from '../utils/errorHandling';
import { getSupabaseClient } from '../services/supabaseService';

interface ClassManagerProps {
  hasSupabase: boolean;
  institutionId?: string;
  initialClassId?: string;
  onBack?: () => void;
}

const ClassManager: React.FC<ClassManagerProps> = ({ hasSupabase, institutionId, initialClassId, onBack }) => {
  const { 
      classes, institutions, loading, error, 
      addClass, updateClass, deleteClass, restoreClass, 
      isAdmin, showDeleted, setShowDeleted, refresh 
  } = useClassManager(hasSupabase, institutionId);
  const { grades } = useSettingsManager(hasSupabase, institutionId);
  
  const [view, setView] = useState<'list' | 'detail' | 'student' | 'professor' | 'grade' | 'test'>(initialClassId ? 'detail' : 'list');
  const [selectedClassId, setSelectedClassId] = useState<string | null>(initialClassId || null);
  const [showForm, setShowForm] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  
  // Filter and Pagination State
  const [filterGrade, setFilterGrade] = useState<string>('');
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(50);
  
  // Navigation IDs for drill-down views
  const [selectedStudentId, setSelectedStudentId] = useState<string | null>(null);
  const [selectedProfessorId, setSelectedProfessorId] = useState<string | null>(null);
  const [selectedGradeId, setSelectedGradeId] = useState<string | null>(null);
  const [selectedTestId, setSelectedTestId] = useState<string | null>(null);
  
  // Navigation handlers
  const handleViewStudent = (studentId: string) => {
    setSelectedStudentId(studentId);
    setView('student');
  };
  
  const handleViewProfessor = (professorId: string) => {
    setSelectedProfessorId(professorId);
    setView('professor');
  };
  
  const handleViewGrade = (gradeId: string) => {
    setSelectedGradeId(gradeId);
    setView('grade');
  };
  
  const handleViewTest = (testId: string) => {
    setSelectedTestId(testId);
    setView('test');
  };
  
  const handleBackToClass = () => {
    setView('detail');
    setSelectedStudentId(null);
    setSelectedProfessorId(null);
    setSelectedGradeId(null);
    setSelectedTestId(null);
  };
  
  // Manager mode and context
  const isManagerMode = !!institutionId || institutions.length === 1;
  const [selectedInstId, setSelectedInstId] = useState(institutionId || '');
  const hasInstitutionContext = isManagerMode || !!selectedInstId;

  // Form State
  const [formData, setFormData] = useState<{name: string, grade_id: string, institution_id: string}>({
      name: '',
      grade_id: '',
      institution_id: institutionId || ''
  });

  // Modal State
  const [modalConfig, setModalConfig] = useState<{
      isOpen: boolean;
      id: string | null;
      action: 'delete' | 'restore';
      name: string;
  }>({ isOpen: false, id: null, action: 'delete', name: '' });
  const [isActionLoading, setIsActionLoading] = useState(false);

  // Auto-select institution if prop provided
  useEffect(() => {
      if (institutionId) {
          setSelectedInstId(institutionId);
          setFormData(prev => ({ ...prev, institution_id: institutionId }));
      } else if (institutions.length === 1) {
          setSelectedInstId(institutions[0].id);
          setFormData(prev => ({ ...prev, institution_id: institutions[0].id }));
      }
  }, [institutions, institutionId]);

  // Derived Data for Creation/Edit
  const availableGrades = useMemo(() => {
      const activeInstId = institutionId || formData.institution_id;
      if (!activeInstId) return [];
      return grades.filter(g => g.institution_id === activeInstId && !g.deleted);
  }, [grades, formData.institution_id, institutionId]);

  // Filter classes by institution, grade and deleted status
  const filteredClasses = useMemo(() => {
      let filtered = classes;
      
      const activeInstId = institutionId || selectedInstId;
      if (activeInstId) {
          filtered = filtered.filter(c => c.institution_id === activeInstId);
      }
      
      // Filter by grade
      if (filterGrade) {
          filtered = filtered.filter(c => c.grade_id === filterGrade);
      }
      
      // Filter by deleted status based on showDeleted flag
      if (!showDeleted) {
          filtered = filtered.filter(c => !c.deleted);
      }
      
      return filtered;
  }, [classes, selectedInstId, institutionId, showDeleted, filterGrade]);

  // Pagination calculations
  const totalPages = Math.ceil(filteredClasses.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const paginatedClasses = useMemo(() => {
      return filteredClasses.slice(startIndex, endIndex);
  }, [filteredClasses, startIndex, endIndex]);

  // Reset page when filters change
  useEffect(() => {
      setCurrentPage(1);
  }, [selectedInstId, filterGrade]);

  // Filter grades for filter dropdown
  const filterGrades = useMemo(() => {
      const activeInstId = institutionId || selectedInstId;
      if (!activeInstId) return [];
      return grades.filter(g => g.institution_id === activeInstId && !g.deleted);
  }, [grades, institutionId, selectedInstId]);

  const handleInstChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
      const id = e.target.value;
      setSelectedInstId(id);
      setFormData(prev => ({ ...prev, institution_id: id, grade_id: '' }));
  };

  const handleSave = async (e: React.FormEvent) => {
      e.preventDefault();
      if (!formData.institution_id) return alert("Por favor, selecione uma instituição");
      if (!formData.grade_id) return alert("Por favor, selecione uma série");
      if (!formData.name) return alert("O nome da turma é obrigatório");
      
      setIsSubmitting(true);
      try {
          const supabase = getSupabaseClient();
          const previousGradeId = editingId ? classes.find(c => c.id === editingId)?.grade_id : null;
          const isGradeChanged = editingId && previousGradeId !== formData.grade_id;
          const isNewClass = !editingId;

          if (editingId) {
              await updateClass(editingId, { 
                  name: formData.name, 
                  grade_id: formData.grade_id, 
                  institution_id: formData.institution_id 
              });
          } else {
              await addClass({ 
                  name: formData.name, 
                  grade_id: formData.grade_id, 
                  institution_id: formData.institution_id 
              });
          }

          // After saving, check if there are disciplines with professors for this grade
          if (supabase && (isNewClass || isGradeChanged)) {
              // Get the class ID (for new classes, we need to fetch it)
              let classId = editingId;
              if (!classId && isNewClass) {
                  // Wait a bit for the insert to complete, then fetch the newly created class
                  await new Promise(resolve => setTimeout(resolve, 100));
                  const { data: newClass } = await supabase
                      .from('classes')
                      .select('id')
                      .eq('name', formData.name)
                      .eq('grade_id', formData.grade_id)
                      .eq('institution_id', formData.institution_id)
                      .eq('deleted', false)
                      .order('created_at', { ascending: false })
                      .limit(1)
                      .maybeSingle();
                  
                  if (newClass) classId = newClass.id;
              }

              if (classId) {
                  // Find disciplines with professors for this grade
                  const { data: disciplines, error: discError } = await supabase
                      .from('disciplines')
                      .select('id, name, professor_id, professors(id, app_users(first_name, last_name))')
                      .eq('grade_id', formData.grade_id)
                      .eq('deleted', false)
                      .not('professor_id', 'is', null);

                  if (!discError && disciplines && disciplines.length > 0) {
                      // Check which professors are not already linked to this class
                      const professorIds = disciplines.map(d => d.professor_id).filter(Boolean) as string[];
                      
                      const { data: existingLinks } = await supabase
                          .from('class_professors')
                          .select('professor_id')
                          .eq('class_id', classId)
                          .in('professor_id', professorIds);

                      const existingProfessorIds = new Set((existingLinks || []).map((l: any) => l.professor_id));
                      const professorsToLink = disciplines.filter(d => d.professor_id && !existingProfessorIds.has(d.professor_id));

                      if (professorsToLink.length > 0) {
                          // Build confirmation message with discipline and professor names
                          const disciplinesList = professorsToLink.map(d => {
                              const prof = d.professors as any;
                              const profName = prof?.app_users 
                                  ? `${prof.app_users.first_name} ${prof.app_users.last_name}`.trim()
                                  : 'Professor';
                              return `• ${d.name} - ${profName}`;
                          }).join('\n');

                          const confirmMessage = `Esta turma pertence à série que possui ${professorsToLink.length} disciplina(s) com professor(es) vinculado(s):\n\n${disciplinesList}\n\nDeseja vincular automaticamente ${professorsToLink.length > 1 ? 'esses professores' : 'este professor'} à turma?`;

                          if (window.confirm(confirmMessage)) {
                              const linksToInsert = professorsToLink.map(d => ({
                                  professor_id: d.professor_id,
                                  class_id: classId
                              }));

                              const { error: linkError } = await supabase
                                  .from('class_professors')
                                  .insert(linksToInsert);

                              if (linkError) {
                                  console.error("Error linking professors to class:", linkError);
                                  alert("Erro ao vincular professores à turma. Tente novamente.");
                              } else {
                                  // Refresh the class list to show the new links
                                  refresh();
                              }
                          }
                      }
                  }
              }
          }

          setShowForm(false);
          setFormData({ name: '', grade_id: '', institution_id: institutionId || selectedInstId || '' });
          setEditingId(null);
      } finally {
          setIsSubmitting(false);
      }
  };

  const handleEdit = (c: SchoolClass) => {
      setEditingId(c.id);
      setFormData({ 
          name: c.name, 
          grade_id: c.grade_id, 
          institution_id: c.institution_id 
      });
      setShowForm(true);
      window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const handleCancel = () => {
      setShowForm(false);
      setEditingId(null);
      setFormData({ name: '', grade_id: '', institution_id: institutionId || selectedInstId || '' });
  };

  // Modal Triggers
  const openDeleteModal = (c: SchoolClass) => {
      setModalConfig({ isOpen: true, id: c.id, action: 'delete', name: c.name });
  };

  const openRestoreModal = (c: SchoolClass) => {
      setModalConfig({ isOpen: true, id: c.id, action: 'restore', name: c.name });
  };

  const executeAction = async () => {
      if (!modalConfig.id) return;
      setIsActionLoading(true);
      try {
          if (modalConfig.action === 'delete') {
              await deleteClass(modalConfig.id);
          } else {
              await restoreClass(modalConfig.id);
          }
          setModalConfig({ ...modalConfig, isOpen: false });
      } catch (err: any) {
          alert(getFriendlyErrorMessage(err));
      } finally {
          setIsActionLoading(false);
      }
  };

  if (!hasSupabase) return <div className="p-8 text-center text-slate-500">Configure o banco de dados primeiro.</div>;

  // Render drill-down views
  if (view === 'student' && selectedStudentId) {
      return (
        <StudentManager 
          hasSupabase={hasSupabase} 
          institutionId={institutionId}
          initialStudentId={selectedStudentId}
          onBack={handleBackToClass}
        />
      );
  }

  if (view === 'professor' && selectedProfessorId) {
      return (
        <ProfessorManager 
          hasSupabase={hasSupabase} 
          institutionId={institutionId}
          initialProfessorId={selectedProfessorId}
          onBack={handleBackToClass}
        />
      );
  }

  if (view === 'grade' && selectedGradeId) {
      return (
        <GradeManager 
          hasSupabase={hasSupabase} 
          institutionId={institutionId}
          initialGradeId={selectedGradeId}
          onBack={handleBackToClass}
        />
      );
  }

  if (view === 'test' && selectedTestId) {
      return (
        <TestManager 
          hasSupabase={hasSupabase}
          initialTestId={selectedTestId}
          onBack={handleBackToClass}
        />
      );
  }

  if (view === 'detail' && selectedClassId) {
      const selectedClass = classes.find(c => c.id === selectedClassId);
      const handleBack = () => { 
            if (onBack) {
              onBack();
            } else {
              setView('list'); 
              setSelectedClassId(null);
            }
      };
      
      return (
        <div>
            <ClassDetails 
              classId={selectedClassId} 
              hasSupabase={hasSupabase} 
              onBack={handleBack}
          onViewStudent={handleViewStudent}
          onViewProfessor={handleViewProfessor}
          onViewGrade={handleViewGrade}
          onViewTest={handleViewTest}
        />
        </div>
      );
  }

  return (
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
          <div>
              <h2 className="text-3xl font-bold text-slate-900 dark:text-slate-100">Turmas</h2>
              <p className="text-slate-500 dark:text-slate-400 mt-1">Gerencie as turmas e classes das instituições</p>
          </div>
          <div className="flex gap-3 items-center">
              {isAdmin && (
                  <label className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300 cursor-pointer select-none bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 px-3 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                      <input 
                          type="checkbox" 
                          checked={showDeleted} 
                          onChange={e => setShowDeleted(e.target.checked)} 
                          className="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500 border-gray-300"
                      />
                      <span className="font-bold">Mostrar Excluídos</span>
                  </label>
              )}
              {hasInstitutionContext && (
                  <button onClick={() => { setShowForm(!showForm); if (showForm) handleCancel(); }} className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-indigo-200 transition-all">
                      {showForm ? 'Cancelar' : <><Plus size={20}/> Nova Turma</>}
                  </button>
              )}
          </div>
      </div>

      {/* Institution Selector for Admin */}
      {!isManagerMode && (
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Contexto da Instituição</label>
          <div className="relative">
            <Building2 size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500"/>
            <select 
              value={selectedInstId} 
              onChange={(e) => {
                handleInstChange(e);
                setFilterGrade(''); // Reset grade filter when institution changes
              }}
              className="w-full border border-slate-300 dark:border-slate-600 rounded-lg pl-10 pr-4 py-2 text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer"
            >
              <option value="">Selecione a Instituição</option>
              {institutions.map(i => <option key={i.id} value={i.id}>{i.name}</option>)}
            </select>
          </div>
        </div>
      )}

      {/* Filters Section - Only show when institution is selected */}
      {hasInstitutionContext && !showForm && (
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            <Users size={16} className="text-indigo-600 dark:text-indigo-400"/>
            <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200 uppercase">Filtros de Listagem</h3>
            <span className="text-xs text-slate-500 dark:text-slate-400 ml-auto">
              {filteredClasses.length} turma{filteredClasses.length !== 1 ? 's' : ''} encontrada{filteredClasses.length !== 1 ? 's' : ''}
            </span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Série</label>
              <div className="relative">
                <BookOpen size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500"/>
                <select 
                  value={filterGrade} 
                  onChange={e => setFilterGrade(e.target.value)} 
                  className="w-full border border-slate-300 dark:border-slate-600 rounded-lg pl-10 pr-4 py-2 text-sm text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer"
                >
                  <option value="">Todas as Séries</option>
                  {filterGrades.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}
                </select>
              </div>
            </div>
            <div className="flex items-end">
              <button
                onClick={() => {
                  setFilterGrade('');
                }}
                className="w-full bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 px-4 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors"
              >
                <X size={16}/> Limpar Filtros
              </button>
            </div>
          </div>
        </div>
      )}

      {/* No Institution Selected Message */}
      {!hasInstitutionContext && !loading && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-8 text-center">
          <Building2 size={48} className="mx-auto text-amber-400 dark:text-amber-500 mb-4"/>
          <h3 className="text-lg font-bold text-amber-800 dark:text-amber-300 mb-2">Nenhuma Instituição Selecionada</h3>
          <p className="text-amber-600 dark:text-amber-400 text-sm">Selecione uma instituição acima para visualizar e gerenciar as turmas.</p>
        </div>
      )}

      {error && (
          <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl p-4 flex gap-3 text-red-700 dark:text-red-300 items-center">
              <AlertTriangle size={20}/> {error}
          </div>
      )}

      <ConfirmationModal
          isOpen={modalConfig.isOpen}
          onClose={() => setModalConfig({ ...modalConfig, isOpen: false })}
          onConfirm={executeAction}
          title={modalConfig.action === 'delete' ? "Excluir Turma" : "Restaurar Turma"}
          message={
              modalConfig.action === 'delete'
              ? <span>Excluir <strong>{modalConfig.name}</strong>? Esta é uma exclusão suave.</span>
              : <span>Restaurar <strong>{modalConfig.name}</strong>?</span>
          }
          confirmLabel={modalConfig.action === 'delete' ? "Excluir" : "Restaurar"}
          isDestructive={modalConfig.action === 'delete'}
          isLoading={isActionLoading}
      />

      {hasInstitutionContext && showForm && (
          <form onSubmit={handleSave} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-4">
              <div className="p-6 bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-700 mb-6">
                  <h3 className="font-bold text-lg text-slate-800 dark:text-slate-100">{editingId ? 'Editar Turma' : 'Cadastrar Nova Turma'}</h3>
              </div>

              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-6">
                      <div>
                          <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Nome da Turma *</label>
                          <input 
                              required 
                              value={formData.name} 
                              onChange={e => setFormData({...formData, name: e.target.value})} 
                              className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100"
                              placeholder="ex: Turma 10-A"
                          />
                      </div>
                  </div>

                  <div className="space-y-6">
                      <div className="bg-indigo-50 dark:bg-indigo-900/30 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800">
                          <h5 className="font-bold text-indigo-900 dark:text-indigo-200 mb-3 text-sm flex items-center gap-2"><Building2 size={16} className="text-indigo-600 dark:text-indigo-400"/> Vínculo Institucional</h5>
                          <div className="space-y-4">
                              <div>
                                  <label className="block text-xs font-bold text-indigo-700 dark:text-indigo-300 uppercase mb-1">1. Instituição</label>
                                  {isManagerMode ? (
                                      <div className="w-full border border-indigo-200 dark:border-indigo-700 bg-white dark:bg-slate-700 rounded-lg px-4 py-2 text-indigo-900 dark:text-indigo-200 flex items-center gap-2">
                                          <Lock size={14} className="text-indigo-600 dark:text-indigo-400"/>
                                          <span className="font-medium">{institutions.find(i => i.id === (institutionId || selectedInstId))?.name || "Instituição Atual"}</span>
                                      </div>
                                  ) : (
                                      <div className="relative">
                                          <Building2 size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-indigo-400 dark:text-indigo-500"/>
                                          <select 
                                              required 
                                              value={formData.institution_id} 
                                              onChange={e => { 
                                                  setFormData({ ...formData, institution_id: e.target.value, grade_id: '' }); 
                                                  setSelectedInstId(e.target.value);
                                              }} 
                                              className="w-full border border-indigo-200 dark:border-indigo-700 rounded-lg pl-10 pr-4 py-2 text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer"
                                          >
                                              <option value="">-- Selecionar Instituição --</option>
                                              {institutions.map(i => <option key={i.id} value={i.id}>{i.name}</option>)}
                                          </select>
                                      </div>
                                  )}
                              </div>
                              <div>
                                  <label className="block text-xs font-bold text-indigo-700 dark:text-indigo-300 uppercase mb-1">2. Série</label>
                                  <div className="relative">
                                      <BookOpen size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-indigo-400 dark:text-indigo-500"/>
                                      <select 
                                          required 
                                          value={formData.grade_id} 
                                          onChange={e => setFormData({ ...formData, grade_id: e.target.value })} 
                                          disabled={!formData.institution_id}
                                          className="w-full border border-indigo-200 dark:border-indigo-700 rounded-lg pl-10 pr-4 py-2 text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer disabled:bg-slate-100 dark:disabled:bg-slate-800 disabled:text-slate-400 dark:disabled:text-slate-500"
                                      >
                                          <option value="">-- Selecionar Série --</option>
                                          {availableGrades.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}
                                      </select>
                                  </div>
                                  {!formData.institution_id && <p className="text-xs text-indigo-400 dark:text-indigo-500 mt-1 italic">Selecione uma instituição primeiro para ver as séries.</p>}
                              </div>
                          </div>
                      </div>
                  </div>
              </div>
              <div className="p-6 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-700 flex justify-end">
                  <button 
                    type="submit" 
                    disabled={isSubmitting}
                    className="bg-indigo-600 dark:bg-indigo-700 hover:bg-indigo-700 dark:hover:bg-indigo-600 text-white px-8 py-3 rounded-lg font-bold flex gap-2 items-center shadow-lg shadow-indigo-200 dark:shadow-indigo-900/50 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                      {isSubmitting ? <Loader2 size={20} className="animate-spin" /> : <CheckCircle size={20}/>}
                      {isSubmitting ? 'Salvando...' : (editingId ? 'Atualizar Turma' : 'Criar Turma')}
                  </button>
              </div>
          </form>
      )}

      {/* List - Omitir durante cadastro */}
      {hasInstitutionContext && !showForm && (
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
          <table className="w-full text-left table-fixed">
          <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
              <tr>
                  <th className="p-4 text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider w-1/3">Nome da Turma</th>
                  <th className="p-4 text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider w-1/4">Série</th>
                  <th className="p-4 text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider w-1/3">Instituição</th>
                  <th className="p-4 text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider text-right w-24">Ações</th>
              </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {loading ? (
                  <tr><td colSpan={4} className="p-12 text-center text-slate-400 dark:text-slate-500"><Loader2 className="animate-spin inline mr-2"/> Carregando turmas...</td></tr>
              ) : filteredClasses.length === 0 ? (
                  <tr><td colSpan={4} className="p-12 text-center text-slate-400 dark:text-slate-500">Nenhuma turma encontrada. Cadastre uma acima.</td></tr>
              ) : paginatedClasses.map(c => {
                  const isDeleted = c.deleted;
                  return (
                      <tr 
                          key={c.id} 
                          onClick={() => { setSelectedClassId(c.id); setView('detail'); }} 
                          className={`transition-colors cursor-pointer group ${isDeleted ? 'bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 border-l-4 border-l-red-400 dark:border-l-red-600' : 'hover:bg-slate-50 dark:hover:bg-slate-700'}`}
                      >
                          <td className="p-4">
                              <div className={`font-semibold text-slate-900 dark:text-slate-100 ${isDeleted ? 'line-through text-red-700 dark:text-red-400' : ''} truncate`}>{c.name}</div>
                              {isDeleted && <span className="text-[10px] text-red-500 font-bold uppercase">Excluído</span>}
                          </td>
                          <td className="p-4">
                              {c.school_grades ? (
                                  <span className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-bold uppercase ${isDeleted ? 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400' : 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300'}`}>
                                      <BookOpen size={12}/> {c.school_grades.name}
                                  </span>
                              ) : (
                                  <span className="text-slate-300 dark:text-slate-600 italic text-xs">Não atribuída</span>
                              )}
                          </td>
                          <td className="p-4 text-sm text-slate-600 dark:text-slate-400 truncate">
                              {c.institutions?.name || 'N/A'}
                          </td>
                          <td className="p-4 text-right">
                              <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                  {isDeleted ? (
                                      <button onClick={(e) => {e.stopPropagation(); openRestoreModal(c)}} className="p-2 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 rounded transition-all" title="Restaurar">
                                          <RotateCcw size={18}/>
                                      </button>
                                  ) : (
                                      <>
                                          <button onClick={(e) => { e.stopPropagation(); handleEdit(c); }} className="p-2 text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded transition-all"><Edit2 size={18}/></button>
                                          <button onClick={(e) => { e.stopPropagation(); setSelectedClassId(c.id); setView('detail'); }} className="p-2 text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded transition-all" title="Ver Detalhes"><Eye size={18}/></button>
                                          <button onClick={(e) => { e.stopPropagation(); openDeleteModal(c); }} className="p-2 text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded transition-all"><Trash2 size={18}/></button>
                                      </>
                                  )}
                              </div>
                          </td>
                      </tr>
                  );
              })}
          </tbody>
          </table>
      </div>
      )}

      {/* Pagination Controls */}
      {hasInstitutionContext && !showForm && filteredClasses.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            {/* Items per page selector and info */}
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <span className="text-sm text-slate-600 dark:text-slate-300 font-medium">Itens por página:</span>
                <select
                  value={itemsPerPage}
                  onChange={(e) => {
                    setItemsPerPage(Number(e.target.value));
                    setCurrentPage(1);
                  }}
                  className="border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-1.5 text-sm text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer"
                >
                  <option value={25}>25</option>
                  <option value={50}>50</option>
                  <option value={100}>100</option>
                  <option value={200}>200</option>
                </select>
              </div>
              <span className="text-sm text-slate-500 dark:text-slate-400">
                Mostrando {startIndex + 1} - {Math.min(endIndex, filteredClasses.length)} de {filteredClasses.length} turma{filteredClasses.length !== 1 ? 's' : ''}
              </span>
            </div>

            {/* Page navigation */}
            <div className="flex items-center gap-2">
              <button
                onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))}
                disabled={currentPage === 1}
                className="p-2 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                title="Página anterior"
              >
                <ChevronLeft size={18} />
              </button>

              {/* Page numbers */}
              <div className="flex items-center gap-1">
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  let pageNum: number;
                  if (totalPages <= 5) {
                    pageNum = i + 1;
                  } else if (currentPage <= 3) {
                    pageNum = i + 1;
                  } else if (currentPage >= totalPages - 2) {
                    pageNum = totalPages - 4 + i;
                  } else {
                    pageNum = currentPage - 2 + i;
                  }

                  return (
                    <button
                      key={pageNum}
                      onClick={() => setCurrentPage(pageNum)}
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                        currentPage === pageNum
                          ? 'bg-indigo-600 text-white'
                          : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-600'
                      }`}
                    >
                      {pageNum}
                    </button>
                  );
                })}
              </div>

              <button
                onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))}
                disabled={currentPage === totalPages}
                className="p-2 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                title="Próxima página"
              >
                <ChevronRight size={18} />
              </button>

              {/* Jump to page */}
              <div className="flex items-center gap-2 ml-2 pl-2 border-l border-slate-300 dark:border-slate-600">
                <span className="text-xs text-slate-500 dark:text-slate-400">Ir para:</span>
                <input
                  type="number"
                  min={1}
                  max={totalPages}
                  value={currentPage}
                  onChange={(e) => {
                    const page = Math.max(1, Math.min(totalPages, Number(e.target.value) || 1));
                    setCurrentPage(page);
                  }}
                  className="w-16 border border-slate-300 dark:border-slate-600 rounded-lg px-2 py-1.5 text-sm text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none text-center"
                />
                <span className="text-xs text-slate-500 dark:text-slate-400">/ {totalPages}</span>
              </div>
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default ClassManager;
